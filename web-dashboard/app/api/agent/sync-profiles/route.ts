...
import { $Enums, VpnCertificateStatus } from '@/lib/generated/prisma';
...
export async function POST(req: Request) {
  ...
  try {
    ...
    await prisma.$transaction(async (tx: {
        vpnUser: {
          upsert: (arg0: {
            where: { username: string; }; update: { status: $Enums.VpnCertificateStatus; serialNumber: string | null | undefined; expirationDate: Date | null; revocationDate: Date | null; ovpnFileContent: string | null; }; // Update these fields if user exists
            create: { // Create with these fields
              isActive: boolean; // Default to inactive, report-status will update if active
              // ovpnFileContent: null, // DIHAPUS: Ini akan menimpa konten OVPN dari commonData
              createdAt: Date; status: $Enums.VpnCertificateStatus; serialNumber: string | null | undefined; expirationDate: Date | null; revocationDate: Date | null; ovpnFileContent: string | null; username: string; nodeId: string;
            };
          }) => Promise<any>;
        };
      }) => {
