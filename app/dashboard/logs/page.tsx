...
export default function LogsPage() {
  ...
  const fetchActionLogs = useCallback(async () => { setIsLoading(true); try { const response = await fetch("/api/logs"); if (!response.ok) throw new Error("Gagal mengambil action logs"); setActionLogs(await response.json()); } catch (error) { toast({ title: "Error", description: "Gagal memuat action logs.", variant: "destructive" }); } finally { setIsLoading(false); } }, [toast]);
  const fetchVpnActivityLogs = useCallback(async () => { setIsVpnLogLoading(true); try { const response = await fetch("/api/activity-logs"); if (!response.ok) throw new Error("Gagal mengambil user activity logs"); setVpnActivityLogs(await response.json()); } catch (error) { toast({ title: "Error", description: "Gagal memuat user activity logs.", variant: "destructive" }); } finally { setIsVpnLogLoading(false); } }, [toast]);
  const fetchNodesForSelect = useCallback(async () => { try { const response = await fetch("/api/nodes"); if (!response.ok) throw new Error("Gagal mengambil node"); setNodes(await response.json()); } catch (error) { toast({ title: "Error", description: "Gagal memuat node untuk filter.", variant: "destructive" }); } }, [toast]);
