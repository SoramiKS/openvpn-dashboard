...
export default function LogsPage() {
  ...
  const [vpnActivityLogs, setVpnActivityLogs] = useState<
    ExtendedVpnActivityLog[]
  >([]);
  ...
  const handleDownloadCsv = () => {
    ...
    const rows = filteredVpnActivityLogs.map((log) =>
      [
    ...
    link.setAttribute(
      "download",
      `user-activity-logs-${new Date().toISOString().split("T")[0]}.csv`
    );
  ...
  useEffect(() => {
    fetchVpnActivityLogs(vpnActivityLogPage);
  }, [fetchVpnActivityLogs, vpnActivityLogPage]);
  useEffect(() => {
    fetchNodesForSelect();
  }, [fetchNodesForSelect]);
  ...
  const filteredActionLogs = useMemo(
    () =>
      actionLogs.filter(
        (log) =>
          (actionLogFilter.nodeId === "all" ||
            log.nodeId === actionLogFilter.nodeId) &&
          (actionLogFilter.action === "all" ||
            log.action === actionLogFilter.action)
      ),
    [actionLogs, actionLogFilter]
  );
  const filteredVpnActivityLogs = useMemo(
    () =>
      vpnActivityLogs.filter((log) => {
        ...
        return (
          (vpnActivityLogFilter.nodeId === "all" ||
            log.nodeId === vpnActivityLogFilter.nodeId) &&
          (vpnActivityLogFilter.action === "all" ||
            log.action === vpnActivityLogFilter.action) &&
  ...
  const totalVpnActivityLogPages = Math.ceil(
    totalVpnActivityLogs / LOGS_PER_PAGE
  );
                  ...
                  filteredActionLogs.map((log) => (
                    <TableRow key={log.id}>
                      ...
                      <TableCell
                        ...
                        title={log.message || log.details || ""}
                      >
                        {log.vpnUser?.username
            ...
            <PaginationContent>
              <PaginationItem>
                <PaginationPrevious
                  ...
                  onClick={(e) => {
                    e.preventDefault();
                    if (actionLogPage > 1) setActionLogPage(actionLogPage - 1);
                  }}
                />
              </PaginationItem>
              <PaginationItem>
              ...
              <PaginationItem>
                <PaginationNext
                  ...
                  onClick={(e) => {
                    e.preventDefault();
                    if (actionLogPage < totalActionLogPages)
                      setActionLogPage(actionLogPage + 1);
                  }}
                />
              </PaginationItem>
            </PaginationContent>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Button
              ...
              disabled={filteredVpnActivityLogs.length === 0}
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 pt-4">
            ...
            <Popover>
              ...
              <PopoverContent className="w-auto p-0">
                <Calendar
                  mode="single"
                  selected={vpnActivityLogFilter.startDate}
                  onSelect={(date) =>
                    setVpnActivityLogFilter((prev) => ({
                      ...prev,
            ...
            <Popover>
              ...
              <PopoverContent className="w-auto p-0">
                <Calendar
                  mode="single"
                  selected={vpnActivityLogFilter.endDate}
                  onSelect={(date) =>
                    setVpnActivityLogFilter((prev) => ({
                      ...prev,
            ...
            <Table>
              ...
              <TableBody>
                {filteredVpnActivityLogs.length === 0 ? (
                  ...
                  filteredVpnActivityLogs.map((log) => (
                    <TableRow key={log.id}>
                      <TableCell>
                        {new Date(log.timestamp).toLocaleString()}
                      </TableCell>
                      ...
                      <TableCell>
                        <Badge
                          variant={
                            log.action === "CONNECT" ? "default" : "secondary"
                          }
                        >
                          {log.action}
                        </Badge>
            ...
            <PaginationContent>
              <PaginationItem>
                <PaginationPrevious
                  ...
                  onClick={(e) => {
                    e.preventDefault();
                    if (vpnActivityLogPage > 1)
                      setVpnActivityLogPage(vpnActivityLogPage - 1);
                  }}
                />
              </PaginationItem>
              <PaginationItem>
              ...
              <PaginationItem>
                <PaginationNext
                  ...
                  onClick={(e) => {
                    e.preventDefault();
                    if (vpnActivityLogPage < totalVpnActivityLogPages)
                      setVpnActivityLogPage(vpnActivityLogPage + 1);
                  }}
                />
              </PaginationItem>
